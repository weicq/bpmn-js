 ...
 *    process diagram
 *  * re-layout connections on re-connect
 *
 ...
 */
function BpmnSnapping(eventBus, canvas, bpmnRules, elementRegistry, layouter) {
  ...
  /**
   * Relayout connection on reconnectStart / reconnectEnd.
   */
  eventBus.on([ 'bendpoint.move.move' ], HIGHER_PRIORITY, function(event) {
    ...
    // only auto-layout on reconnect start or end
    if (context.type === COMMAND_BENDPOINT_UPDATE) {
      return;
    }

    if (!context.oldWaypoints) {
      context.oldWaypoints = connection.waypoints.slice();
    }

    // revert to old
    if (!hover) {
      connection.waypoints = context.oldWaypoints;
      context.updateWaypoints = false;
    ...
    if (context.type === COMMAND_RECONNECT_END) {
      connection.waypoints = context.oldWaypoints;

      waypoints = layouter.layoutConnection(connection, {
        connectionEnd: {
          x: event.x,
          y: event.y
        },
    ...
    if (context.type === COMMAND_RECONNECT_START) {
